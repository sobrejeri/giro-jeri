  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="md:max-w-4xl md:mx-auto md:w-full md:px-6 md:py-6 flex flex-col flex-1">
        ...
        {/* Date + Time */}
        <div className="px-4 md:px-0 pb-3 grid grid-cols-2 gap-2">
          <input
            type="date"
            value={date}
        ...
        {/* Escolha o passeio */}
        <div className="mb-4">
          <div className="px-4 md:px-0 flex items-center justify-between mb-2.5">
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-2xl overflow-hidden border-2 transition-all text-left ${
                        selected ? 'border-brand shadow-md' : 'border-gray-100 hover:border-gray-200'
                      }`}
                    >
                      <div className="h-24 bg-gradient-to-br from-orange-100 to-amber-50 flex items-center justify-center relative">
                        <Icon size={32} className="text-brand/25" />
                        {selected && (
                          <div className="absolute top-2 right-2 w-5 h-5 bg-brand rounded-full flex items-center justify-center">
                            <Check size={11} className="text-white" />
                          </div>
                      ...
                      <div className="p-3 bg-white">
                        <p className="text-sm font-semibold text-gray-900 line-clamp-2 leading-tight">{t.name}</p>
                        {t.duration_hours && (
                          <span className="text-xs text-gray-400 flex items-center gap-0.5 mt-1">
                            <Clock size={10} /> {t.duration_hours}h
                          </span>
                        )}
                        {mode === 'shared' && t.is_shared_enabled && (
        ...
        {/* Bottom bar */}
        <div className="sticky bottom-0 md:static bg-white border-t md:border-t-0 md:border-t border-gray-100 px-4 md:px-0 py-3 flex items-center gap-4 md:mt-4">
          {totalPrice ? (
            <div>
