...
function Field({ label, value, children }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      {children || <span className="text-[14px] text-gray-700">{value || <span className="text-gray-300 italic">Não informado</span>}</span>}
    </div>
  )
}
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50">
                <span className="font-semibold text-gray-800 text-[14px]">Dados pessoais</span>
                {!editing ? (
                  <button
                    onClick={startEdit}
                    className="flex items-center gap-1 text-brand text-[13px] font-medium active:opacity-70"
                  >
              ...
              <div className="px-5 py-4 space-y-4">
                {editing ? (
                  ...
                  <>
                    <div>
                      <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Nome completo</label>
                      <input
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand"
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Telefone / WhatsApp</label>
                      <input
                        type="tel"
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Data de nascimento</label>
                      <input
                        type="date"
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand"
                    ...
                    <div className="flex gap-2">
                      <div className="flex-shrink-0 w-[120px]">
                        <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Tipo doc.</label>
                        <select
                          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand bg-white"
                      ...
                      <div className="flex-1">
                        <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Número</label>
                        <input
                          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand"
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Nacionalidade (código ISO)</label>
                      <input
                        placeholder="BR"
                        maxLength={2}
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand uppercase"
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1 block">Gênero</label>
                      <select
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[14px] text-gray-800 focus:outline-none focus:border-brand bg-white"
                        value={form.gender}
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Telefone" value={user.phone && <span className="flex items-center gap-1"><Phone size={11} className="text-gray-400" />{user.phone}</span>} />
                      <Field label="Nascimento" value={user.birth_date ? new Date(user.birth_date + 'T12:00:00').toLocaleDateString('pt-BR') : null} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Documento" value={docLabel && user.document_number ? `${docLabel}: ${user.document_number}` : (docLabel || user.document_number)} />
                      <Field label="Nacionalidade" value={user.nationality && <span className="flex items-center gap-1"><Flag size={11} className="text-gray-400" />{user.nationality}</span>} />
                    </div>
                    <Field label="Gênero" value={genderLabel} />
                    {(user.emergency_contact_name || user.emergency_contact_phone) && (
                      <div className="pt-2 border-t border-gray-50 space-y-1">
                        <p className="flex items-center gap-1.5 text-[11px] font-bold text-orange-500">
                          <AlertCircle size={12} /> Contato de emergência
                        </p>
                        <Field label="Nome" value={user.emergency_contact_name} />
                        <Field label="Telefone" value={user.emergency_contact_phone} />
                      </div>
                    )}
                  </>
                )}
