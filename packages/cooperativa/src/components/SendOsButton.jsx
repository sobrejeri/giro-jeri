...
export default function SendOsButton({ booking, form, cooperativa, variant = 'inline' }) {
  ...
  async function send() {
    ...
    try {
      // Envia o LINK da OS (não o arquivo). O cliente e o motorista abrem no
      // navegador e baixam o PDF por lá, se quiserem. Antes o PDF ia em base64
      // e estourava o limite de corpo da API ("request entity too large").
      const r = await api.sendOsLink(booking.id)
      if (r?.skipped) throw new Error('WhatsApp não está configurado.')
      setState('ok')
      setMsg('Link da OS enviado ao cliente e ao motorista.')
    } catch (e) {
      setState('error')
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className={`text-xs truncate ${state === 'error' ? 'text-red-600' : 'text-green-600'}`}>
              {msg || 'Envia o link ao cliente e ao motorista'}
            </p>
          </div>
        </button>
      </>
