...
const intentSchema = z.object({
  ...
  payment_method:      z.enum(['pix', 'credit_card', 'debit_card', 'nupay']).default('pix'),
...
async function finalizeNupayPayment(payment, providerPayment) {
  ...
  // Estes dois caminhos têm índices/upserts próprios e podem ser repetidos para
  // reparar uma falha transitória depois que a aprovação atômica já ocorreu.
  await recordLegAccounting(booking, payment).catch((err) =>
    console.error('[nupay] contabilidade por perna falhou payment=%s code=%s', payment.id, err.code || 'ledger_error'))
  await recordAffiliateCommission(booking).catch((err) =>
    console.error('[nupay] comissão de afiliado falhou payment=%s code=%s', payment.id, err.code || 'commission_error'))

  if (finalized) {
    sendConfirmationEmail(booking).catch((err) =>
      console.error('[nupay] e-mail de confirmação falhou payment=%s code=%s', payment.id, err.code || 'email_error'))
...
async function completeNupaySession(payment, requestedSessionId) {
  ...
  if (!currentPayment.gateway_transaction_id) {
    return { ...currentPayment, ...sessionUpdates, status: 'pending' }
  }
...
async function computeChargedTotal({ data, userId, strict = false }) {
  const {
  ...
  try {
    ...
    if (strict && !authoritative) {
      throw new Error('Não foi possível calcular o preço autoritativo')
    }
  } catch (e) {
    if (strict) throw e
...
router.post('/intent', authenticate, async (req, res, next) => {
  try {
    ...
    const requestedNupay = payment_method === 'nupay'
    ...
    let chargedTotal = existing_booking_id
      ? Number(total_price)
      : isGroup ? 0 : (await computeChargedTotal({
        data: parsed.data,
        userId: req.user.id,
        strict: requestedNupay,
      })).total

    // ── 1. Lê configurações do gateway ─────────────────
    const cfg     = await getPaymentSettings()
    const gateway = requestedNupay ? 'nupay' : (cfg.payment_gateway || 'manual')
    console.log('[intent] gateway=%s env=%s', gateway, cfg.payment_gateway_env)

    if (requestedNupay) {
      if (isGroup) {
        return res.status(422).json({
          error: 'NuPay ainda não está disponível para pagamento agrupado. Pague cada reserva separadamente.',
        })
      }
    ...
    if (requestedNupay) {
      ...
      const { data: payment, error: pErr } = await supabase
        ...
        .insert({
          ...
          gateway_fee_amount: calcFee(chargedTotal, cfg.payment_nupay_fee_percent),
          currency: 'BRL',
          status: 'pending',
...
// Polling: limitado ao dono da reserva.
router.get('/:id/status', authenticate, async (req, res, next) => {
  try {
    if (!isUuid(req.params.id)) return res.status(400).json({ error: 'Pagamento inválido' })
    const { data: payment } = await supabase
      .from('payments')
      .select('id, status, booking_id, order_group_id, gateway_name, gateway_transaction_id, gateway_checkout_url, provider_session_id, provider_status, expires_at, amount_gross, gateway_fee_pct, gateway_fee_amount, bookings!inner(booking_code, status_commercial, operator_id, user_id)')
      .eq('id', req.params.id)
      .eq('bookings.user_id', req.user.id)
      .maybeSingle()
...
// A notificação é apenas um gatilho; o estado real sempre é consultado na NuPay.
