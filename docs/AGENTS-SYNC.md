...
- **2026-07-23 · Codex (merge da plataforma + NuPay Sessions seguro, migrations
  061–063)** — A branch `claude/giro-jeri-platform-GFBFR` foi integrada à
  `codex/nupay-payment-flow`, preservando carrinho, motor de pernas, split MP,
  afiliados, i18n e fluxos atuais. NuPay usa Sessões oficiais: intent com
  `Idempotency-Key`, preço autoritativo do servidor, redirect oficial, retorno
  restaurável por `payment_id`, polling e webhooks que consultam a NuPay, e
  aprovação somente em `COMPLETED`. Status e cancelamento são limitados ao dono;
  estorno integral exige admin. Credenciais vivem somente em
  `NUPAY_APP_KEY`/`NUPAY_APP_TOKEN`; settings públicos expõem apenas booleano
  efetivo e o admin mostra configurado/não configurado. A tentativa recusada,
  cancelada ou expirada não encerra a reserva, permitindo fallback para PIX.
  Migrations renumeradas para evitar colisão: **061** método/eventos, **062**
  limpeza de segredos legados, **063** IDs/índices e funções transacionais de
  aprovação/estorno. NuPay permanece oculto com `NUPAY_ENABLED=false` até
  credenciais, URLs HTTPS e sandbox aprovados. Primeiro release não aceita
  `order_group_id` nem split nativo NuPay. Runbook: `docs/NUPAY.md`.
