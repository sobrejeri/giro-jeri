...
| 0.7 | **NuPay permanece OFF até homologação** | ✅ | Código integrado; manter `NUPAY_ENABLED=false` e `payment_nupay_enabled=false` até aplicar migrations 061–063, cadastrar URLs HTTPS e concluir sandbox |
...
| 1.9 | NuPay reserva única | 🔴 | Sandbox: criar, aprovar, recusar, cancelar, expirar e estornar; repetir retorno + polling + webhook concorrentes e conferir 1 aprovação/ledger/e-mail |
| 1.10 | NuPay segurança e fallback | 🔴 | Testar valor adulterado, outro usuário, `412`, `409`, `429`, timeout, refresh no retorno e troca para PIX; grupo deve responder `422` |
...
- NuPay para pagamento agrupado e split nativo (primeiro release aceita reserva única)
